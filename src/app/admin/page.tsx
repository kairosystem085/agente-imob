...
export default function AdminPage() {
  ...
  async function loadOrganizations() {
    ...
    if (!brokerForm.organization_id && data.organizations[0]) {
      setBrokerForm((current) => ({ ...current, organization_id: data.organizations[0].id }));
    }
  }
  ...
  async function createOrganization(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setMessage("Nao foi possivel criar o cliente.");
      return;
    }
  ...
  async function createBroker(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    ...
    if (!response.ok) {
      setMessage("Nao foi possivel criar o acesso.");
      return;
    }
  ...
  const totalProperties = organizations.reduce((sum, organization) => sum + organization.properties_count, 0);
  const totalLeads = organizations.reduce((sum, organization) => sum + organization.leads_count, 0);
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <nav className="flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3">
            ...
            <div><p className="font-semibold">ImobIA</p><p className="text-xs text-slate-400">Modo agencia</p></div>
          </Link>
        ...
        <section className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-lg border border-white/10 bg-[#1a1f2e] p-6 shadow-sm">
            ...
            <p className="mt-4 max-w-3xl text-sm leading-6 text-slate-300">Cadastre corretores e imobiliarias, gere acessos, acompanhe imoveis publicados e conecte o WhatsApp de cada operacao.</p>
          </div>
          ...
          <div className="rounded-lg border border-white/10 bg-[#1a1f2e] p-6 shadow-sm">
            ...
            <div className="mt-6 grid grid-cols-3 gap-3 text-sm">
              <div className="rounded-md bg-white/5 p-3"><p className="text-slate-400">Imoveis</p><p className="mt-1 text-xl font-semibold">{totalProperties}</p></div>
              <div className="rounded-md bg-white/5 p-3"><p className="text-slate-400">Leads</p><p className="mt-1 text-xl font-semibold">{totalLeads}</p></div>
              <div className="rounded-md bg-white/5 p-3"><p className="text-slate-400">WhatsApp</p><p className="mt-1 text-xl font-semibold">{connectedInstances}/{organizations.length}</p></div>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-lg border border-white/10 bg-[#1a1f2e] p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold">Clientes ativos</h2><button type="button" onClick={loadOrganizations} className="rounded-md bg-emerald px-4 py-2 text-sm font-semibold text-[#0f1117]">Atualizar lista</button></div>
              ...
              <tbody className="divide-y divide-white/10">
                {organizations.map((organization) => (
                  <tr key={organization.id}>
                    <td className="px-4 py-4 font-semibold">{organization.name}</td><td className="px-4 py-4 text-slate-300">/{organization.slug}</td><td className="px-4 py-4 text-slate-300">{organization.type}</td><td className="px-4 py-4 text-slate-300">{organization.instancia_status ?? "closed"}</td><td className="px-4 py-4 text-slate-300">{organization.properties_count}</td><td className="px-4 py-4 text-slate-300">{organization.leads_count}</td><td className="px-4 py-4"><StatusBadge status={organization.plan} /></td><td className="px-4 py-4"><Link href={`/catalogo/${organization.slug}`} className="text-emerald">Catalogo</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="rounded-lg border border-white/10 bg-[#1a1f2e] p-6 shadow-sm">
          ...
          <div className="mt-5 space-y-3">{organizations.map((organization) => <div key={organization.id} className="flex items-center justify-between rounded-md bg-white/5 p-4"><div><p className="font-semibold">{organization.slug}</p><p className="text-sm text-slate-400">Status: {organization.instancia_status ?? "closed"}</p></div><button type="button" className="rounded-md border border-white/10 px-3 py-2 text-sm text-emerald">Forcar reconexao</button></div>)}</div>
        </section>
      </div>
    </main>
  );
